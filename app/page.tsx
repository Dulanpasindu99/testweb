...
export default function MedLinkDoctorDashboard() {
  ...
  const normalizeDiseaseSuggestions = (payload: unknown, query: string): string[] => {
    ...
    const possibleTables: unknown[] = [payload[3], payload[2], payload[1]].filter(Array.isArray);

    const names = possibleTables.flatMap((table) =>
      (table as unknown[])
        .filter((entry): entry is unknown[] => Array.isArray(entry))
        .map((entry) => entry.filter((part) => typeof part === 'string').join(' — '))
    );
    ...
    const prefixMatches = uniqueNames.filter((name) => name.toLowerCase().startsWith(qLower));
    const fallbackMatches = prefixMatches.length
      ? prefixMatches
      : uniqueNames.filter((name) => name.toLowerCase().includes(qLower));
